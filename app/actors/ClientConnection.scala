...
class ClientConnection(username: String, uid: String, upstream: ActorRef,frontendManager: ActorRef) extends Actor {
  ...
  def receive = {
        ...
        case "new_game" =>
          ...
          newGameResult match {
            case s: JsSuccess[NewGameJSON] => 
              ...
              val future = frontendManager ? NewGame(s.get.name.replaceAll(" ", "_") + "__" + System.currentTimeMillis(),s.get.n_players,user_info,self)
              future.onSuccess {
    ...
    case MessageCode(uid,code) =>
      var g_json = new MessageCodeJSON("message",code)
